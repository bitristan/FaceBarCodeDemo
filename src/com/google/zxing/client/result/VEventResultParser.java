...
public final class VEventResultParser extends ResultParser {
	...
	public CalendarParsedResult parse(Result result) {
		...
		String location = matchSingleVCardPrefixedField("LOCATION", rawText,
				true);
		String organizer = stripMailto(matchSingleVCardPrefixedField(
				"ORGANIZER", rawText, true));
		...
		String description = matchSingleVCardPrefixedField("DESCRIPTION",
				rawText, true);
		...
		if (geoString == null) {
			...
			try {
				latitude = Double
						.parseDouble(geoString.substring(0, semicolon));
				longitude = Double.parseDouble(geoString
						.substring(semicolon + 1));
		...
		try {
			return new CalendarParsedResult(summary, start, end, duration,
					location, organizer, attendees, description, latitude,
					longitude);
	...
	private static String matchSingleVCardPrefixedField(CharSequence prefix,
		...
		List<String> values = VCardResultParser.matchSingleVCardPrefixedField(
                prefix, rawText, trim, false);
		return values == null || values.isEmpty() ? null : values.get(0);
	}

	private static String[] matchVCardPrefixedField(CharSequence prefix,
			String rawText, boolean trim) {
		List<List<String>> values = VCardResultParser.matchVCardPrefixedField(
                prefix, rawText, trim, false);
